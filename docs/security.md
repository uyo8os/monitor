...
- `user_version` 不能高于本版本的 `SCHEMA_VERSION`（来自更新的 hub，降级读不了）
- page size 必须和当前库一致——WAL 模式下在线备份 API 不接受页大小变化，这条是把
  `SQLITE_READONLY` 换成一句人话
- **迁移在这里跑，不在覆盖之后。** 版本低于本版本的备份，那几条迁移是在**上传的那个文件上**执行
  的（`check_backup` 因此会写 `src`）。原来放在覆盖之后：迁移失败时活库已经被换掉，接口却回一句
  「恢复失败」——「恢复失败」和「你的数据没了」同时成立的唯一一种排法
- **八张表的列也要对得上，不只是表名。** 逐页复制意味着文件带的列就是 hub 接下来每条语句面对的
  列，而「八个表名齐全、列全不对」的文件能过上面每一道闸，然后把库变成不可用。比对的对象是本
  版本用 `SCHEMA` 现建的一个内存库，所以**不存在第二份要同步的列清单**；比的是列名集合而不是
  建表语句原文——迁移过的老备份是靠 `ALTER TABLE` 到齐的，DDL 文本永远不会和新建的一样。多出来
  的列不管
...
**收满之后先改名，再读。** 上传路径是固定的一个名字（协议里没有 upload id，见
[architecture.md](architecture.md#分片上传)），所以第二次上传能落在同一个文件上。它可以毁掉一次
上传——那是已知上限；不能允许的是它在 `check_backup` 读完之后、逐页复制之前继续往里写：SQLite 看
不见另一个文件句柄做的修改，于是一份过了全部闸门的文件会以**另一个状态**被覆盖上去。改名之后那次
上传的下一片找不到文件，收到的是它本来就有的那句「这次上传已经不在了，请从头开始」。主题上传同理。

**没修的那一半**：两个标签页同时上传**同一类**文件（分片大小相同，偏移天然对齐，长度检查分辨不
出归属）仍然会拼接，代价是这次上传失败。要连它一起修就得给协议加上传身份，而那正是
[decisions.md](decisions.md#上传走分片协议只有-offset-和-total-用户) 明确否决的东西——它会把「孤儿
文件没人回收」请回来。
...
- 探测目标必须是 `host:port` 格式；间隔必须落在 5–3600 秒内，**越界是 400 而不是 clamp**——agent
  那边还会再 clamp 一次，所以静默取边界不会失败，只会悄悄变成另一个数，而低于下限的那个数是
  5 秒，也就是最快的探测，跑在每一个被分配的节点上。同一条规矩见 `retention_days`
...
- **在飞的历史查询数 clamp 到 4（`api::HISTORY_GATE`），满了直接 503，且整段查询在
  `spawn_blocking` 里跑。** 上面那条说的全是「一次请求多贵」，而这条路匿名可达、一次请求整段占着
  agent 上报那把写锁，所以还得说清「同时能有几个」。没有这道闸时实测：一台机器 120 个并发请求把
  面板自己的 `GET /api/nodes` 从 1 ms 拖到 2.8 s，且随并发数线性增长；内存没事（峰值 25 MB），被
  吃光的是锁。拒绝而不排队，理由同密码闸门。这是同一个洞的第三次——`PASSWORD_GATE` 和 `RELAY_GATE`
  各堵过一次，而「单次上界」那条自查项恰好看不见它。

  **第一版写成 8，而且没挪出 runtime，量出来是零拒绝。** handler 不 await，permit 就只在 worker
  线程真正跑着它时被持有，三核机器上最多同时 3 个，闸门写 8 永远不触发（实测：120 并发、0 个 503、
  面板 14 s）。密码闸门那条「值必须小于机器真能同时跑的数量」是同一个坑的另一面：这里不是值太大，
  是 handler 拿不住那么多 permit。挪进 `spawn_blocking` 之后 permit 跨 await 持有，「在飞」才是字面
  意思——改完实测 120 并发里 104 个被挡，面板 1.27 s（debug 构建）
- `/agent/{arch}` 边收边转，不把整个 release 收进内存再发；并发转发数 clamp 到 `RELAY_SLOTS = 4`
  ...
  是这个进程上单次最贵的匿名操作。**permit 不挂在 handler 上**：响应头几微秒就建好了，真正的开销
  在后面那 1.8 MB。**也不挂在响应体上**——「传完为止」是客户端说了算：接下来不读的客户端让 hyper
  冲不出去，hyper 于是不再 poll 这个流，写在 `poll_next` 里的截止时间也就永远不会被检查（reqwest
  那 120 秒上游超时同样是 poll 驱动的，一样不会触发）。四条收下响应就不读的连接能把四个 permit
  占到连接断开为止，而这条路是每台节点装机的必经之路。所以 permit 交给一个**自己带定时器**的任务
  （`main::RELAY_DEADLINE`，180 秒），响应体这端一被丢弃就提前放行
...
- [ ] 那条路**同时能有几个在飞**？单次上界说不清这件事：`/api/nodes/{id}/metrics` 的单次上界一直
      写得很清楚，照样能被 120 个并发请求把 hub 拖垮。闸门要么是并发数（`HISTORY_GATE`、
      `RELAY_GATE`、`PASSWORD_GATE`），要么是持有时长（`RELAY_DEADLINE`），通常两个都要
