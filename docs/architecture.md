...
| **[agent](https://github.com/uyo8os/monitor-agent)** | Linux agent。发布自己的 musl 二进制，`install.sh` 从那边的 release 拉 |
...
agent 的采集代码在 [另一个仓库](https://github.com/uyo8os/monitor-agent)。改了它的上报字段就是改了协议，两边要同步。
...
`Metrics` 的字段（[agent 仓库](https://github.com/uyo8os/monitor-agent) 里 `src/collect.rs` 的 `Metrics` struct 就是权威定义）：
