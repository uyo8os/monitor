...
> `install.sh` 直接安装，中间没有摘要校验。只填信得过的镜像。
...
确实要往一台没有 TLS 的 hub 上装，那就手工跑脚本并自己加 `--insecure`——面板不会替你拼这个参数。
...
而且 403 来自边缘，hub 侧一行日志都没有。这一版新增的是 `POST /api/themes`（上传主题）
...
[monitor-theme-default](https://github.com/monitor-probe/monitor-theme-default)。
